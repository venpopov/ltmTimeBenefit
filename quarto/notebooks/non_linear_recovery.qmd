...
## Model predictions (with lambda = 0.5)
...
shinyApp(
  ui = fluidPage(
    ...
    sidebarLayout(
      sidebarPanel(
        sliderInput("prop", "prop:", min = 0, max = 1, value = 0.29),
        sliderInput("prop_ltm", "prop_ltm:", min = 0, max = 1, value = 0.62),
        sliderInput("rate", "rate:", min = 0, max = 1, value = 0.25),
        sliderInput("gain", "gain:", min = 1, max = 100, value = 11),
        sliderInput("tau", "tau:", min = 0, max = 1, value = 0.41),
      ),
      mainPanel(
        plotOutput("distPlot")
      )
  ...
  server = function(input, output) {
    output$distPlot <- renderPlot({
      ...
      data |>
        mutate(
          Probability = predict_resmodel(par, data = data, group_by = c("gap", "chunk"), growth = "asy", lambda = 0.5),
          Strength = predict_resmodel(par, data = data, group_by = c("gap", "chunk"), type = "strength", growth = "asy", lambda = 0.5)
        ) |>
        pivot_longer(c(Probability, Strength), names_to = "type", values_to = "value") |>
        ggplot(aes(gap, value, color = chunk, group = chunk)) +
        geom_line() +
